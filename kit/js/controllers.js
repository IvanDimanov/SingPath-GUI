...
window.triggerHash = function (hash) {
  ...
  for(var savedHash in window.hashes) {
    // Getting all callback functions
    var callbacks = window.hashes[savedHash];
    
    // Chech if there's a parameter hash match
    var parameterHash   = savedHash.split('*');
    var isParameterHash = !hash.indexOf(parameterHash[0]) && parameterHash.length == 2 && parameterHash[1] == '';
    var parameter       = isParameterHash ? hash.replace(parameterHash[0], '') : undefined;
...
// if there's a change in the page address hahs
